...
public class ProjectileControl : NetworkBehaviour
{
    ...
    void Start()
    {
        ...
        if (!IsOwner)
        {
            Debug.Log($"[ProjectileControl] Not owner on ClientId: {NetworkManager.Singleton.LocalClientId}. Disabling script.");
            enabled = false;
            return;
        }

        Debug.Log($"[ProjectileControl] Owner STARTED on ClientId: {NetworkManager.Singleton.LocalClientId}");

        if (IsHost)
        {
            spawnPoint = GameObject.Find("HostSpawnPoint")?.transform;
            Debug.Log("Assigned Host spawn point");
        }
        else if (IsClient)
        {
            spawnPoint = GameObject.Find("ClientSpawnPoint")?.transform;
            Debug.Log("Assigned Client spawn point");
        }
        ...
        if (trajectoryLine == null)
        {
            Debug.LogWarning("Trajectory line not assigned");
        }
    ...
    {
        base.OnNetworkSpawn();
        Debug.Log($"[ProjectileControl] OnNetworkSpawn on Client {NetworkManager.Singleton.LocalClientId} | IsOwner: {IsOwner}");
    ...
    public void PlayAudio(AudioClip audio)
    {
        // Optional: implement audio playback here
    }
}